...
- (void)addEmotion:(BSEmotionModel *)emotionModel
{
    ...
    [self emotionToolbar:self.toolbar didSelectIndex:index];
}
        ...
        if (self.lastIndexPath.section != index) { // 删除的不是选中的
            if (index == count) { // 删除的最后一个
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self.collectionView scrollToItemAtIndexPath:self.lastIndexPath atScrollPosition:UICollectionViewScrollPositionCenteredHorizontally animated:NO];
                });
